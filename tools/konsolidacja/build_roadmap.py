...
RE_SF =re.compile(r'(SCI-FI|FIKCJA|WORLDBUILDING|moonshot)', re.I)
...
def view_of(idx, txt):
    v=['calosc']
    if idx in MOON or RE_SF.search(txt): v.append('moonshoty')
...
"""
JS="""const bs=[...document.querySelectorAll('.blk')];let V='calosc',Q='';
...
i tak też są tu prezentowane.</p></section>
%s
...
<script>%s</script></body></html>"""%(CSS,len(P),TODAY,nav,body,idx_rows,JS)
